 ...
 * Turret will aim at BLUE basket (4, 140) in HARD_LOCK mode.
 * Can see either tag 20 (blue) or 24 (red) for positioning, but always aims at blue basket.
 */
...
public class SoloBlue extends CommandOpMode {
    ...
    public void initialize() {
        ...
        telemetry.addLine("Aiming at BLUE basket (4, 140)");
        telemetry.update();
    }
    ...
    public void run() {
        ...
        if (robot.turret != null && robot.vision != null) {
            ...
            // Only use tx when seeing the BLUE basket tag (our target)
            if (currentTagId == Vision.BLUE_GOAL_TAG_ID) {
                robot.turret.updateTx(robot.vision.getTx(), true);
            } else {
                // Seeing red tag or no tag - use odometry mode for turret
        ...
        */
        ...
        telemetry.addLine("=== SOLO BLUE ===");
        telemetry.addData("Target", "BLUE basket (4, 140)");
